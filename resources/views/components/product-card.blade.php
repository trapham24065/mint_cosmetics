...
<div class="product-item product-st3-item">
    <div class="product-thumb">
        ...
        $isOnSale = $firstVariant && $firstVariant->discount_price && $firstVariant->discount_price < $firstVariant->price;
            $discountPercent = $isOnSale && $firstVariant->price > 0
            ? round((($firstVariant->price - $firstVariant->discount_price) / $firstVariant->price) * 100)
            : 0;
            @endphp
            @if($isOnSale)
            <span class="badge bg-danger position-absolute top-0 start-0 m-2">Giảm {{ $discountPercent }}%</span>
            @endif
    ...
    <div class="product-info">
        ...
        @if($product->brand)
        <div class="text-muted small mb-1">Thương hiệu: {{ $product->brand->name }}</div>
        @endif
        <div class="prices">
