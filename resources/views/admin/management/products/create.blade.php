                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <small class="text-muted">
                                        Chỉ danh mục con cuối cùng (danh mục lá) mới dùng để tạo sản phẩm.
                                    </small>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    >
                                        ...
                                        @foreach($brands as $brand)
                                            <option
                                                value="{{ $brand->id }}"
                                                @selected((int) old('brand_id') === $brand->id)
                                            >
                                                {{ $brand->name }}
                                            </option>
                                        @endforeach
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4 mb-3">
                                        <label for="discount-percentage" class="form-label">Tỷ lệ chiết khấu
                                            (%)</label>
                                        <input type="number" step="any" min="0" max="100" id="discount-percentage"
    ...
    @push('scripts')
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const oldVariants = Array.isArray(oldVariantsRaw) ? oldVariantsRaw : Object.values(
                    oldVariantsRaw || {});
                const oldSelectedValueIds = new Set(
                    oldVariants.flatMap(variant => {
                        ...
                        let valuesHtml = attr.values.map(val => `
                            <div class="form-check form-check-inline">
                                <input class="form-check-input attribute-value-check" type="checkbox" id="value-${val.id}"
                                    data-attribute-id="${attr.id}" value="${val.id}" ${oldSelectedValueIds.has(
                            String(val.id)) ? 'checked' : ''}>
                ...
                function renderOldVariantForms (attributes) {
                    ...
                    oldVariants.forEach((variant, index) => {
                        ...
                        const variantName = valueIds.map(id => valueLabelMap.get(String(id)) || `#${id}`).join(' / ') ||
                            `Biến thể ${index + 1}`;
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.addEventListener('input', function(event) {
                    if (!event.target.classList.contains('variant-price') &&
                        !event.target.classList.contains('variant-discount-percentage')) {
                        return;
                    }
                ...
                document.querySelectorAll('.variant-form-row .card-body, .card.mb-3.bg-light-subtle .card-body').
                    forEach(row => {
