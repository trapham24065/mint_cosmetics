        ...
        <tr>
            ...
            <td class="reason-cell">
                <div
                    ...
                    data-admin-note="{{ $return->admin_note }}"
                ...
                <div class="mt-2">
                    <small class="text-danger fw-semibold">Phản hồi từ shop:</small>
                    <div
                        class="small text-danger reason-text"
                        data-bs-toggle="modal"
                        data-bs-target="#reasonModal"
                        data-reason="{{ $return->reason }}"
                        data-description="{{ $return->description }}"
...
<div
    class="d-flex flex-wrap justify-content-between align-items-center gap-2 mt-3 pt-2 border-top dashboard-pagination">
    ...
    <div class="dashboard-pagination__actions" role="group" aria-label="Returns pagination">
        <button type="button" class="btn btn-sm btn-outline-secondary js-dashboard-page-link dashboard-pagination__btn"
            data-page="{{ max(1, $currentPage - 1) }}" {{ $currentPage <= 1 ? 'disabled' : '' }}>&lt;
        </button>
        @for($page = $startPage; $page <= $endPage; $page++)
            <button type="button"
            class="btn btn-sm {{ $page === $currentPage ? 'btn-primary' : 'btn-outline-secondary' }} js-dashboard-page-link dashboard-pagination__btn"
            data-page="{{ $page }}">{{ $page }}</button>
            @endfor
            <button type="button" class="btn btn-sm btn-outline-secondary js-dashboard-page-link dashboard-pagination__btn"
                data-page="{{ min($lastPage, $currentPage + 1) }}" {{ $currentPage >= $lastPage ? 'disabled' : '' }}>
                &gt;
            </button>
    </div>
</div>