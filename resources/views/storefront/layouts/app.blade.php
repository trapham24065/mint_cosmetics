...
<head>
    ...
    <title>Mint Cosmetic </title>
    <meta name="robots" content="noindex, follow" />
    <meta name="description" content="Mint Cosmetic ">
    ...
    <link rel="stylesheet" href="{{asset('assets/storefront/css/components/megamenu.css')}}">

    <link rel="stylesheet" href="{{asset('assets/storefront/css/components/aside-menu.css')}}">
...
<body>
    ...
    <script src="{{ asset('assets/storefront/js/megamenu.js') }}"></script>
    @stack('scripts')
    <x-toast />
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.body.addEventListener('click', function(event) {
                ...
                if (quickViewButton) {
                    ...
                    quickViewContainer.innerHTML = '<div class="d-flex justify-content-center py-5"><div class="spinner-border text-primary" role="status"><span class="visually-hidden">Đang tải...</span></div></div>';
                    quickViewModal.show();
                    ...
                    fetch(`/products/${productId}/quick-view`).then(response => {
                        ...
                        if (!product || !product.name) {
                            throw new Error('Dữ liệu sản phẩm không hợp lệ đã được nhận.');
                        }
            ...
            function renderQuickViewContent(product) {
                const defaultImageUrl = product.image ?
                    ...
                    '/assets/storefront/images/shop/default.webp';
                ...
                const hasDiscount = (product.variants && product.variants.length > 0 &&
                    product.variants[0].discount_price && parseFloat(product.variants[0].discount_price) <
            ...
            <div class="row">
                <div class="col-lg-5 col-md-6 mb-4 mb-md-0">
                    <div class="product-single-thumb border rounded shadow-sm overflow-hidden position-relative">
                        <img id="quickViewImage"
                         ...
                         ${hasDiscount
                ?
                `<span class="position-absolute top-0 start-0 bg-danger text-white px-2 py-1 m-2 rounded small fw-bold">Giảm giá</span>`
                : ''}
                    </div>
                </div>
                ...
                <div class="col-lg-7 col-md-6">
                    <div class="product-details-content ps-lg-3">
                        ...
                        <div class="product-details-review d-flex align-items-center mb-3">
                            ...
                            <span class="text-muted small">(${product.reviews_count || 0} đánh giá)</span>
                        </div>
                ...
                : 'Chọn các tùy chọn để xem giá'}
                            ...
                            <div class="product-details-cart-wishlist">
                                <button type="button" class="btn action-btn-cart" id="quickViewAddToCart" data-context="quick-view" disabled>Thêm vào giỏ hàng</button>
                            </div>
                        </div>
                        ...
                        <div class="product-details-meta mt-4 pt-3 border-top">
                            <ul class="list-unstyled text-muted small mb-0">
                                <li class="mb-1"><span class="fw-bold text-dark">Mã SKU:</span> <span id="quickViewSku">${currentProductVariants.length ===
            1 ? (currentProductVariants[0].sku || 'N/A') : 'N/A'}</span></li>
                                <li><span class="fw-bold text-dark">Danh mục:</span> ${product.category
            ...
            function updateQuickViewVariantDetails() {
                ...
                if (matchedVariant) {
                    ...
                    priceEl.textContent = 'Không khả dụng';
                    addToCartBtn.dataset.variantId = '';
                    addToCartBtn.disabled = true;
                    addToCartBtn.innerHTML = 'Không khả dụng';
                }
            }
        });
    </script>
