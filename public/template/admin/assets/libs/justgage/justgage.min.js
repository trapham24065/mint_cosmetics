    ...
    (!1 !== c &&
      ...
      (e.canvas.customAttributes.pki = function (t, n) {
        ...
        return (
          ...
          (F =
            'M' +
            ((d = a / 2 + r) - (u = (g = a / 2 - a / 10) - (a / 6.666666666666667) * f)) +
            ',' +
            (p = l / 1.25 + c) +
            ' '),
          (F += 'L' + (d - g) + ',' + p + ' '),
        ...
        return (
          ...
          f
            ? ((s = (1 - (2 * (t - n)) / (i - n)) * Math.PI),
              ...
              (v = x - T * Math.cos(s)),
              (y = h + T * Math.sin(s)),
              (z = 'M' + (F = v + C * Math.sin(s)) + ',' + (S = y + C * Math.cos(s)) + ' '),
              (z += 'L' + (w = v - C * Math.sin(s)) + ',' + (A = y - C * Math.cos(s)) + ' '),
              (z += 'L' + b + ',' + M + ' '),
              { path: (z += 'Z ') })
            : ((s = (1 - (t - n) / (i - n)) * Math.PI),
      ...
      (e.gauge = e.canvas
        .path()
        .attr({ stroke: 'none', fill: e.config.gaugeColor, pki: [e.config.max] })),
        ...
        : e.config.humanFriendly
          ...
          : e.config.formatNumber
            ...
            : e.config.displayRemaining
              ? (e.originalValue =
                  (1 * (e.config.max - e.originalValue)).toFixed(e.config.decimals) +
                  e.config.symbol)
              : (e.originalValue =
                  (1 * e.originalValue).toFixed(e.config.decimals) + e.config.symbol),
      !0 === e.config.counter)
    ) {
      const t = function () {
          let t = e.level.attr('pki')[0];
          (e.config.reverse &&
            (t = 1 * e.config.max + 1 * e.config.min - 1 * e.level.attr('pki')[0]),
