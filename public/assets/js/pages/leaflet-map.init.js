var mymap = L.map('Leaf_default').setView([51.505, -0.09], 13),
  popup =
    ...
    L.marker([51.5, -0.09])
      ...
      .openPopup(),
    L.circle([51.508, -0.11], 500, { color: 'red', fillColor: '#f03', fillOpacity: 0.5 })
      .addTo(mymap)
...
function boundsExtendBounds() {
  (rectangle3 && (map.removeLayer(rectangle3), (rectangle3 = null)),
    ...
    (bounds3 = new L.LatLngBounds(bounds1.getSouthWest(), bounds1.getNorthEast())).extend(bounds2),
...
function boundsExtendLatLng() {
  (rectangle3 && (map.removeLayer(rectangle3), (rectangle3 = null)),
    (bounds3 = bounds3 && null),
    (bounds3 = new L.LatLngBounds(bounds1.getSouthWest(), bounds1.getNorthEast())).extend(
      marker.getLatLng()
    ),
    (rectangle3 = new L.Rectangle(bounds3, {
      color: '#ff0000',
...
for (
  var path = new L.Polyline(latlngs), i = 0, latlngs2 = [], len = poly_points.length;
  i < len;
  i++
)
...
for (
  var poly = new L.Polygon(latlngs2),
    ...
    osmAttrib =
      '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors',
    osm = L.tileLayer(osmUrl, { maxZoom: 18, attribution: osmAttrib }),
    map = L.map('Moov_Canvas', { preferCanvas: !0 }).setView([50.5, 30.51], 15).addLayer(osm),
    markers = [],
