class DynamicSelect {
  constructor(e, t = {}) {
    ((this.options = Object.assign(
      ...
      (this.selectElement = 'string' == typeof e ? document.querySelector(e) : e));
    for (const s in this.selectElement.dataset)
      void 0 !== this.options[s] && (this.options[s] = this.selectElement.dataset[s]);
    if (
  ...
  _template() {
    ...
    var e = `
            <div class="dynamic-select ${this.name}"${this.selectElement.id ? ' id="' + this.selectElement.id + '"' : ''} style="${this.width ? 'width:' + this.width + ';' : ''}${this.height ? 'height:' + this.height + ';' : ''}">
                ...
                <div class="dynamic-select-options" style="${this.options.dropdownWidth ? 'width:' + this.options.dropdownWidth + ';' : ''}${this.options.dropdownHeight ? 'height:' + this.options.dropdownHeight + ';' : ''}">${i}</div>
            </div>
        `,
    ...
    (this.element.querySelectorAll('.dynamic-select-option').forEach((t) => {
      t.onclick = () => {
        (this.element
          ...
          (this.element.querySelector('.dynamic-select-header').innerHTML = t.innerHTML),
          (this.element.querySelector('input').value = t.getAttribute('data-value')),
          this.data.forEach((e) => (e.selected = !1)),
          (this.data.filter((e) => e.value == t.getAttribute('data-value'))[0].selected = !0),
          this.element
  ...
  _updateSelected() {
    this.selectedValue &&
      (this.element.querySelector('.dynamic-select-header').innerHTML = this.element.querySelector(
...
document.querySelectorAll('[data-dynamic-select]').forEach((e) => new DynamicSelect(e));
